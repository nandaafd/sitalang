...
<div>
    ...
    <div class="container bg-light py-3 py-5 rounded px-4">
        ...
        @endif
        <form action="{{route('admin.update',$data->user_id)}}" method="post" id="formAddAdmin">
            @method('put')
            @csrf
            <div class="mb-3">
                <label for="range_poin" class="form-label">Nama Lengkap</label>
                <input type="text" class="form-control @error('fullname') is-invalid @enderror" value="{{$data->user->fullname}}" name="fullname" placeholder="Tuliskan nama admin..">
                @error('fullname')
            ...
            <button type="submit" class="btn btn-primary btnAddAdmin"><i class="bi bi-save"></i> Simpan</button>
        </form>
    </div>
</div>
